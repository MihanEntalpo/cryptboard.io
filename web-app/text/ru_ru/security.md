# Безопасность CryptBoard

## Как работает CryptBoard?

* Данные шифруются на клиенте (в браузере) и расшифрованы могут быть только в браузере, и только правильным получателем.
...
* Используется гибридный алгоритм шифрования RSA+AES
* Клиент генерирует асимметричные (публичный и закрытый) ключи RSA
* Вы делитесь публичным ключом с другим пользователем посредством другого канала связи 
    * мессенджер
...
* Для шифрования использованы OpenSource библиотеки, в частности, JSEncrypt и forge. Я сам не настолько крут в криптографии чтобы проанализировать эти библиотеки, 
но предположу что кто-то это уже сделал (позже добавлю информацию об этом) 
* Все файлы взяты из официальных репозиториев, ссылки на них размещены на странице <a href='#about'>Информация</a>
...
компрометация сервера не позволяла взломать ваши сообщения
* Любой может развернуть self-hosted сервер CryptBoard и самостоятельно его контролировать.
...
* Также, вы можете также сверить uid и публичный ключ получателя, что сложнее, но надёжнее.
* Не отправляйте сообщения получателям, публичного ключа которых у вас нет (аватарка с вопросом), по умолчанию это и так отключено. Сообщения таким получателям не шифруются.
* После того как вы закончили работать с сервисом, уничтожьте ваши данные нажав на иконку "Череп с костями" в верхней панели или в гамбургер-меню.
...
* Man-In-The-Middle: если кто-то даст вам не тот ключ и uid что вы ожидали - он сможет выдавать себя за легитимного получателя. Поэтому проверяйте как минимум аватар.
...
* Какие-то ошибки или закладки в сторонних библиотеках: Библиотеки шифрования JSEncrypt и Forge, как и любые opensource-библиотеки могут иметь ошибки и даже закладки позволяющие расшифровывать данные. Здесь мы можем полагаться только на результаты сторонних криптографических анализов этих библиотек 
